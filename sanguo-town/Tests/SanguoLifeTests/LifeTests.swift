...
final class LifeTests: XCTestCase {
    func testCatalog() throws {let c=try LifeCatalog.bundled();XCTAssertEqual(c.heroes.count,30);XCTAssertEqual(c.skills.count,22);XCTAssertEqual(LifeResource.allCases.count,12);XCTAssertEqual(c.recipes.count,4)}
