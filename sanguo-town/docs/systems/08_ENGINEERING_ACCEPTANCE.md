...
- 职责：命令信封、权限校验、原子保存、回执幂等、离线、数据不变量及GC01—GC28验收。
...
- 验收范围：GC01—GC28（合同集合，不代表全部已执行），完整断言见[S08](08_ENGINEERING_ACCEPTANCE.md#sec-9-6)。
...
### 9.6 v0.9运行验收（实际状态见BUILD_STATUS；未覆盖项保持NOT_RUN）
|ID|必须断言|
|---|---|
