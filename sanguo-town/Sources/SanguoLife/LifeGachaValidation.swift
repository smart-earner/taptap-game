...
extension LifeWorld {
    func validateGacha() throws {
        ...
        try check(Set(g.stars.keys).isSubset(of:ids) && (5...30).contains(g.stars.count) && g.stars.values.allSatisfy{(1...5).contains($0)},"武将星级或身份无效")
        ...
        try check(g.arrivals.values.allSatisfy{$0>time},"抵达时间无效")
        ...
        try check(g.minted==consumed["gold_ingot",default:0]/1000*10 && consumed["gold_ingot",default:0]%1000==0,"金币没有完整金锭来源")
        ...
        try check((1...4).contains(g.houseLevels.count) && g.houseLevels.allSatisfy{(1...3).contains($0)},"住宅级别无效")
