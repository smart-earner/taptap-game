...
public struct LifeRuntime: Sendable {
    ...
    func nextEvent() -> Int64 {
        ...
        if let d=husbandryNextEvent() {n=min(n,d)}
    ...
    mutating func assign(kind:String,job:String,subject:String,at node:String,work:Int64,tail:[LifeStep]=[],only:String?=nil) -> String? {
        ...
        for a in available {
            ...
            steps += tail.map { step in
                var s=step
                if s.kind=="work" {s.seconds=max(1,(s.seconds*10000+Int64(r)-1)/Int64(r))}
                return s
            }
    ...
    mutating func finishTask(_ t:LifeTask) {
        switch t.kind {
        case "pig_receive","pig_care","pig_process": finishHusbandryTask(t)
        case "export":
        ...
        case "finish":
            ...
            if r.job=="cook",world.husbandry != nil {
                world.husbandry!.recentMeals.append(r.id=="cook_meat")
                if world.husbandry!.recentMeals.count>8 {world.husbandry!.recentMeals.removeFirst()}
            }
    ...
    public func label(_ location:String) -> String {
        ...
        return ["warehouse":"粮仓","kitchen-in":"厨房","kitchen-out":"厨房出餐台","home-meals":"住宅餐点","hall-meals":"官署餐点","trees":"林地","quarry":"采石点","mine":"矿点","forge-in":"工坊","forge-out":"工坊","recruit":"官署接待点","ration-in":"制粮台","ration-out":"军粮架","pasture-in":"牧栏料槽","meat-out":"肉食台"][location] ?? location
    }
}
